...
class AudioDeviceTemplate : public AudioDeviceGeneric {
 public:
  ...
  virtual ~AudioDeviceTemplate() { WCLOG(INFO) << __FUNCTION__; }
  ...
  int32_t PlayoutDelay(uint16_t& delay_ms) const override {
    ...
    WCLOG(LS_VERBOSE) << __FUNCTION__ << " delay = " << delay_ms;
  ...
  void ClearPlayoutWarning() override { WCLOG(INFO) << __FUNCTION__; }

  void ClearPlayoutError() override { WCLOG(INFO) << __FUNCTION__; }

  void ClearRecordingWarning() override { WCLOG(INFO) << __FUNCTION__; }

  void ClearRecordingError() override { WCLOG(INFO) << __FUNCTION__; }
  ...
  int32_t EnableBuiltInAEC(bool enable) override {
    if (enable) {
      WCLOG(INFO) << __FUNCTION__ << ": enabling built in AEC";
    } else {
      WCLOG(INFO) << __FUNCTION__ << ": disabling built in AEC";
    }
  ...
  int32_t EnableBuiltInAGC(bool enable) override {
    if (enable) {
      WCLOG(INFO) << __FUNCTION__ << ": enabling built in AGC";
    } else {
      WCLOG(INFO) << __FUNCTION__ << ": disabling built in AGC";
    }
  ...
  int32_t EnableBuiltInNS(bool enable) override {
    if (enable) {
      WCLOG(INFO) << __FUNCTION__ << ": enabling built in NS";
    } else {
      WCLOG(INFO) << __FUNCTION__ << ": disabling built in NS";
    }
